...
// decrypt file
func DecryptFile(inputFile, outPutFile string, bPriKey *ecdsa.PrivateKey, capsule *Capsule, pubX *ecdsa.PublicKey) (time5 int64) {
	start5 := time.Now()
	keyBytes, err := decryptKeyGen(bPriKey, capsule, pubX)
	if err != nil {
		fmt.Println(err)
	}
	time5 = time.Since(start5).Microseconds()
