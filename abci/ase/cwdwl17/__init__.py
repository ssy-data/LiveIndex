...
class KPabe_search(ABEnc):    
    ...
    def setup(self):
        ...
        #u_hat, h_hat, w_hat = self.group.random(G2), self.group.random(G2), self.group.random(G2) 
        ...
        pk = { 'g':g, 'g_hat':g_hat, 'u':u, 'h':h, 'w':w, 'g1':g1, 'g2':g2, 'g3':g3, 'g4':g4, 'e_gg_alpha':e_gg_alpha }
        msk = {'alpha':alpha, 'u_hat': u_hat, 'h_hat': h_hat, 'w_hat': w_hat, 'd1':d1, 'd2':d2, 'd3':d3, 'd4':d4 }
    ...
    def trapdoor(self, mk, pk, pk_s, policy_str): 
        ...
        for kw, row in mono_span_prog.items():
            ...
            T1[k] = pk['g_hat'] ** Mivtop * mk['w_hat']**(mk['d1']*mk['d2']*t1+mk['d3']*mk['d4']*t2)
            
            tem = self.group.hash(str(pair(pk_s, T_prime) ** r), ZR)
            
            T2[k] = pk['g_hat'] ** (mk['d1']*mk['d2']*t1+mk['d3']*mk['d4']*t2 + tem)
            T3[k] = ((mk['u_hat'] ** kwHash * mk['h_hat'])**t1)**(-mk['d2'])
            T4[k] = ((mk['u_hat'] ** kwHash * mk['h_hat'])**t1)**(-mk['d1'])
            T5[k] = ((mk['u_hat'] ** kwHash * mk['h_hat'])**t2)**(-mk['d4'])
            T6[k] = ((mk['u_hat'] ** kwHash * mk['h_hat'])**t2)**(-mk['d3'])
            
        return { 'T':T, 'T_prime':T_prime, 'T1':T1, 'T2':T2, 'T3':T3, 'T4':T4, 'T5':T5, 'T6':T6, 'policy':str(policy_stripped) }  
    ...
    def search(self, pk, trap, ct, sk_s):
        ...
        num_keywords = 0
        num_tests = 0
        tep = pk['g_hat'] ** self.group.hash(str(pair(trap['T'], trap['T_prime'])**sk_s), ZR)
        for attr_list in pruned_list:
            #if debug: print("\nKeywords for testing:", attr_list)
            A = 1 
            for i in attr_list:
                j = i.getAttributeAndIndex(); k = i.getAttribute() 
                B = pair(ct['D'], trap['T1'][k]) * pair(ct['D_i'][k], trap['T2'][k]/tep) * pair(ct['E1'][k], trap['T3'][k]) * pair(ct['E2'][k], trap['T4'][k]) * pair(ct['F1'][k], trap['T5'][k]) * pair(ct['F2'][k], trap['T6'][k]) #** z[j]
                A *= B
            num_tests += 1
