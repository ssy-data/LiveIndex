...
func (mem *MemberKey) Sign(cert *Cert, c1, c2, c3 *pbc.Element, m string) *Sig {
	sig := new(Sig)
	sig.msg = m
...
type Data struct {
	GenerateGroup  []int64 `json:"generateGroup"`
	GenerateMember []int64 `json:"generateMember"`
	GenerateSign   []int64 `json:"generateSign"`
	VerifySign     []int64 `json:"verifySign"`
	TraceSign      []int64 `json:"traceSign"`
}
...
func main() {
	for i := 0; i < 10; i++ {
		...
		fmt.Println("time of generate group is", result1[i])
		start2 := time.Now()
		//genarate  new  member
		...
		fmt.Println("time of generate member is", result2[i])
		...
		//generate  signature
		start3 := time.Now()
		data, err := os.ReadFile("test4.json")
		if err != nil {
		...
		sig := member.Sign(cert, c1, c2, c3, string(m))
		result3 = append(result3, time.Since(start3).Microseconds())
		fmt.Println("time of generate sign is", result3[i])
		...
		fmt.Println("time of verify sign is", result4[i])
		// priv.Group.Verify_sign(sig1, member1.h_)
		start5 := time.Now()
		priv.open(sig)
		result5 = append(result5, time.Since(start5).Microseconds())
		fmt.Println("time of trace sign is", result5[i])
		// priv.open(sig1)
	}
	data := Data{
		GenerateGroup:  result1,
		GenerateMember: result2,
		GenerateSign:   result3,
		VerifySign:     result4,
		TraceSign:      result5,
	}
